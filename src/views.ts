...
const repositoryAction = (repository: Repository, csrfToken: string) => {
  ...
  return `<form id="repository-action-${escapeHtml(repository.githubId)}" class="flex flex-wrap items-center gap-3" action="${action}" method="post" hx-post="${action}" hx-target="this" hx-swap="none" hx-indicator="#repository-progress-${escapeHtml(repository.githubId)}" hx-disabled-elt="button[type='submit']">
    <input type="hidden" name="csrf" value="${escapeHtml(csrfToken)}">
