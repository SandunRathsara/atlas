...
This repository contains the runnable Atlas slice: private sign-in, durable Repository/Spec/Session records, read-only Pull request/native-stack browsing, authenticated preparation, a guarded OpenCode handoff, and public release discovery/staging.
...
The global `/updates` page checks public GitHub Releases at startup, every four hours, and through **Check now**. Production staging is handled by the independently bootstrapped updater service through `ATLAS_UPDATER_SOCKET` and `ATLAS_UPDATER_KEY_PATH`; local development without that service reports staging as unavailable rather than changing an active release. This slice downloads and verifies complete release trees only. It does not expose activation or change Session admission.
