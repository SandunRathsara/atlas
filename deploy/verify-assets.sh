...
for script in "$root/bin/gh" "$root/bin/git-credential-atlas" "$root/check-health.sh" "$root/stage-release.sh"; do
  bash -n "$script"
done
...
if command -v systemd-analyze >/dev/null 2>&1; then
  ...
  if ! systemd-analyze verify "$root/systemd/atlas.service" "$root/systemd/opencode.service" >"$unit_output" 2>&1; then
    ...
    unexpected=$(grep -Ev '^((atlas|opencode)\.service: Command /opt/atlas/tools/(bun/1\.3\.14/bin/bun|opencode/0\.0\.0-beta-19135/bin/opencode2) is not executable: No such file or directory)$' "$unit_output" || true)
