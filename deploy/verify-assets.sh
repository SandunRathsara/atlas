...
for script in "$root/bin/gh" "$root/bin/git" "$root/bin/git-credential-atlas" "$root/capture-recovery-config.sh" "$root/check-health.sh" "$root/check-opencode.sh" "$root/check-space.sh" "$root/stage-opencode.sh" "$root/stage-release.sh"; do
  bash -n "$script"
done
...
if command -v systemd-analyze >/dev/null 2>&1; then
  ...
  if ! systemd-analyze verify "$root/systemd/atlas.service" "$root/systemd/opencode.service" >"$unit_output" 2>&1; then
    ...
    unexpected=$(grep -Ev '^((atlas|opencode)\.service: Command /opt/atlas/(current/deploy/(bin/git|check-opencode\.sh)|tools/(bun/1\.3\.14/bin/bun|git/2\.55\.0/bin/git|opencode/0\.0\.0-beta-19135/bin/opencode2)) is not executable: No such file or directory)$' "$unit_output" || true)
